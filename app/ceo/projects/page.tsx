    ...
    <div className="relative min-h-screen py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-white">Project Portfolio Management</h1>
          <p className="text-xs text-slate-400 mt-1">Monitor active engineering projects, sprint milestones, and capital utilization.</p>
        </div>
          ...
          <Card key={p.id} glow glowColor="cyan" className="p-6 flex flex-col justify-between space-y-4">
            <div className="space-y-3">
              ...
              <h3 className="text-lg font-bold text-white hover:text-cyan-300 transition-colors">
                <Link href={`/ceo/projects/${p.slug || p.id}`}>{p.name}</Link>
              </h3>
            ...
            <div className="pt-3 border-t border-slate-800 space-y-2 text-xs">
              <div className="flex justify-between text-slate-400">
                <span>Budget: <strong className="text-emerald-400 font-mono">${p.budget?.toLocaleString()}</strong></span>
                <span>Spent: <strong className="text-amber-400 font-mono">${p.spent?.toLocaleString() || 0}</strong></span>
              </div>
              <Link href={`/ceo/projects/${p.slug || p.id}`} className="block">
                <Button variant="outline" size="sm" className="w-full text-xs">
                  Inspect Project Desk →
                </Button>
              </Link>
            </div>
          </Card>
