...
export default function CgoAuditLogsPage() {
  const [logs, setLogs] = useState<any[]>([]);

  useEffect(() => {
    async function load() {
      const res = await fetch("/api/cgo/audit-logs");
  ...
  return (
    <div className="relative min-h-screen py-12 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto space-y-8">
      ...
      <div className="pb-4 border-b border-slate-800">
        ...
        <h1 className="text-3xl font-extrabold text-white">Innovation Governance Audit Logs</h1>
        <p className="text-xs text-slate-400 mt-1">Immutable record of all idea transitions, CGO reviews, and executive routing events.</p>
      </div>
            ...
            <div key={l.id} className="p-4 rounded-xl bg-slate-900 border border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="font-mono font-bold text-cyan-400">{l.action}</span>
