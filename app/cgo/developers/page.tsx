...
export default function CgoDevelopersPage() {
  const [devs, setDevs] = useState<any[]>([]);

  useEffect(() => {
    async function load() {
      const res = await fetch("/api/cgo/developers");
  ...
  return (
    <div className="relative min-h-screen py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="pb-4 border-b border-slate-800">
        ...
        <h1 className="text-3xl font-extrabold text-white">Developer Squad Availability</h1>
        <p className="text-xs text-slate-400 mt-1">Match engineering skillsets with validated community innovation problems.</p>
      </div>
          ...
          <Card key={i} glow glowColor="cyan" className="p-6 space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-cyan-400 font-mono">{d.position || "Developer"}</p>
              </div>
            </div>
            <p className="text-xs text-slate-400">{d.company || "Xynvora AI Core Team"}</p>
            <div className="pt-3 border-t border-slate-800 flex justify-between text-xs text-slate-500">
              ...
              <span>Rep: <strong className="text-cyan-400 font-mono">{d.reputation_score || 100}</strong></span>
            </div>
          </Card>
        ))}
      </div>
